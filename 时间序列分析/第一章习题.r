...
Myfunc <- function(data1,number){
	...
	result1 <- matrix(0,6,4,dimnames=list(c("均值","标准差","偏度","超额峰度","最小值","最大值"),c(a[1],a[2],a[3],a[4]))) 
	result2 <- matrix(0,6,4,dimnames=list(c("均值","标准差","偏度","超额峰度","最小值","最大值"),c(paste("log(",a[1],"+1",")"),paste("log(",a[2],"+1",")"),paste("log(",a[3],"+1",")"),paste("log(",a[4],"+1",")"))))

	n <- nrow(data1)

	#第1问：简单收益率各项统计指标计算
	for(k in 1:4){
	  result1[1,k] <- mean(data1[,k])
	  result1[2,k] <- sd(data1[,k])
	  result1[3,k] <- Ske(data1[,k])
	  result1[4,k] <- Kur(data1[,k])
	  result1[5,k] <- min(data1[,k])
	  result1[6,k] <- max(data1[,k])
	}

	#第2问：对数收益率各项统计指标计算
	for(k in 1:4){
	  result2[1,k] <- mean(data2[,k])
	  result2[2,k] <- sd(data2[,k])
	  result2[3,k] <- Ske(data2[,k])
	  result2[4,k] <- Kur(data2[,k])
	  result2[5,k] <- min(data2[,k])
	  result2[6,k] <- max(data2[,k])
	}
